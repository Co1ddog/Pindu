    ...
    <el-dialog v-model="dialogVisible" :title="title" width="50%">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="handleClose(ruleFormRef)">发送</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
<script lang="ts" setup>
  ...
  const data = []
  onBeforeMount(() => {
    feedbackListFromAftersale().then((r) => {
      r.data.forEach((e, i) => {
        data.push(e)
      })
  ...
  const del = (row) => {
    ...
    ElMessageBox.confirm('你确定要删除当前项吗?', '温馨提示', {
      ...
      .then(async () => {
        await feedbackDelete(row.id).then((r) => {
