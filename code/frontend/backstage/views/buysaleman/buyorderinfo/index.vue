...
<script lang="ts" setup name="comprehensive">
  ...
  import { getbuyorderdetail } from '../../../api/orderDetails'
  const load = ref(false)
  const load1 = ref(true)
  const data = []
  const books = ref([])
  onMounted(() => {
    getbuyorders().then((r) => {
  ...
  function edit(row) {
    ...
    title.value = '详细信息'
    rowObj.value = row
    getbuyorderdetail(row.id).then((r) => {
      console.log(row.id)
      console.log(r)
      for (let i = 0; i < r.data.length; i++) {
        books.value.push({
          b_isbn: r.data[i].book_isbn,
          b_name: r.data[i].book_name,
          customer_condition: r.data[i].customer_condition,
          final_condition: r.data[i].final_condition,
        })
      }
      console.log(books)
  ...
  const column = [
    ...
    { name: 'id', label: '买书订单号', inSearch: true, valueType: 'input', width: 180 },
  ...
  const column1 = [
    { name: 'b_isbn', label: 'ISBN', inSearch: true },
