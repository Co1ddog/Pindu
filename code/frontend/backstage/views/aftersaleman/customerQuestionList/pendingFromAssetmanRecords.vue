    ...
    <el-dialog v-model="dialogVisible" :title="title" width="50%">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="handleClose(ruleFormRef)">发送给用户</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
<script lang="ts" setup>
  ...
  import { feedbackListPendingFromAsset, feedbackUpdate } from '../../../api/feedbackService'
  ...
  const data = []
  onBeforeMount(() => {
    feedbackListPendingFromAsset().then((r) => {
      r.data.forEach((e, i) => {
        data.push(e)
      })
  ...
  const column = [
    ...
    {
      name: 'que_content_assetman2aftersaleman',
      label: '资金专员回复',
      inSearch: true,
      valueType: 'input',
    },
        ...
        list.value.forEach((item) => {
          if (item.id === rowObj.value.id) {
            ...
            feedbackUpdate(item.id, {
              que_content_aftersaleman2user: obj.que_content_aftersaleman2user,
              que_condition: 3,
